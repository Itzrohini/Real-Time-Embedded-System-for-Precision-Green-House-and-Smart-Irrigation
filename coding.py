...
base = "http://192.168.137.178/"
esp32_cam_url = "http://192.168.137.184/capture"  # ESP32-CAM URL
...
def get_sensor_data():
    try:
        ...
        if len(values) == 5:
            ...
            f"https://api.thingspeak.com/update?api_key=GNRSRJK8C2GWOPAC&field1={v1}&field2={c1}&field3={ch}&field4={dh}&field5={additional_value}&field6={plant_health_percentage}&field7={class_label}"
        )
        conn.read()
        conn.close()
...
def send_to_cloud(v1, c1, ch, dh, additional_value, health):
    try:
        conn = urllib.request.urlopen(
            f"https://api.thingspeak.com/update?api_key=GNRSRJK8C2GWOPAC&field1={v1}&field2={c1}&field3={ch}&field4={dh}&field5={additional_value}&field6={health}"
        )
        conn.read()
        conn.close()
    except Exception as e:
...
def display_remedy(plant_health_percentage):
    if plant_health_percentage >= 80:
        remedy = "Plant is healthy. No action needed."
    elif 50 <= plant_health_percentage < 80:
        remedy = "Plant needs moderate care. Water and fertilize."
    else:
        remedy = "Plant in critical condition. Immediate attention required."

    remedy_label.config(text=f'Remedy: {remedy}')

# Function to check overall health by comparing CNN and KNN results
def check_overall_health(sensor_health_status):
    global image_health_status  

    if image_health_status == "healthy" and sensor_health_status == "healthy":
        overall_health_label.config(text="Overall Health: Healthy")
    else:
        overall_health_label.config(text="Overall Health: Unhealthy")
